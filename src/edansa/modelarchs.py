...
class WaveToTensor(object):
    ...
    def __init__(self,
        ...
        self.mel_n_fft = 2048
        self.mel_hop_length = 512
        if feature_method == 'logmel':
            self.mel_power = 2.0
        self.mel_n_mels = 128
        self.mel_n_mfcc = 40
        self.mel_window = 'hann'
        ...
        if feature_method == 'logmel':
            self.melspect_transform = torchaudio.transforms.MelSpectrogram(
                ...
                n_fft=self.mel_n_fft).to(device)
            ...
            raise ValueError(f'feature_method not recognized {feature_method}')

    def __call__(self, sample):
        x, y = sample
        # print(x.dtype)
        # print('before spect', x.reshape(-1)[0])
        if self.feature_method == 'logmel':
            # torch audio can work from gpu
            # if x is numpy array, convert to tensor
            ...
            x = self.db_transform(mel)
        elif self.feature_method == 'mfcc':
            # torch audio can work from gpu
            # if x is numpy array, convert to tensor
            if isinstance(x, np.ndarray):
                x = torch.from_numpy(x)
            ...
            # normalize to mean 0 std 1
            x = (x - torch.mean(x)) / torch.std(x)
            x = self.mfcc_transform(x.reshape(-1))

        else:
            raise ValueError(
                f'feature_method not implemented {self.feature_method}')

        x = x[:, :self.maxMelLen]
        if x.shape[1] > self.maxMelLen:
...
class DropStripes(object):
    ...
    def transform_slice(self, e, total_width):
        ...
        for _ in range(self.stripes_num):
            ...
            bgn = torch.randint(low=0, high=total_width - distance,
                                size=(1,))[0]
...
class SpecAugmentation(object):
    ...
    def __init__(self, time_drop_width, time_stripes_num, freq_drop_width,
        ...
        self.freq_dropper = DropStripes(dim=2,
                                        drop_width=time_drop_width,
                                        stripes_num=time_stripes_num)

        self.time_dropper = DropStripes(dim=1,
...
class Cnn6(nn.Module):
    ...
    def __init__(self,
        ...
        self.last_conv_block_out_chan = last_conv_block_out_chan
        ...
        self.conv_block4 = ConvBlock5x5(
            in_channels=256, out_channels=self.last_conv_block_out_chan)
        self.fc1_output_size = 128
        if self.global_pool_type == '':
            self.fc1 = nn.Linear(self.last_conv_block_out_chan * 58,
                                 self.last_conv_block_out_chan,
                                 bias=True)
        else:
            self.fc1 = nn.Linear(self.last_conv_block_out_chan,
                                 self.fc1_output_size,
                                 bias=True)
        self.fc_audioset = nn.Linear(self.fc1_output_size,
                                     classes_num,
                                     bias=True)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = self.conv_block0(x,
                             pool_size=(2, 2),
                             pool_type=self.intermediate_pool_type)
        ...
        # print('input shape after mean dim=3', x.shape)
        if self.global_pool_type == 'avg+max':
            ...
            raise Exception(
                f'this global pool type not implemented{self.global_pool_type}')
